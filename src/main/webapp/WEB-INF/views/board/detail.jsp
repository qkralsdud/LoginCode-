...
<div class="container">
	...
	<c:if test="${sessionScope.principal.id == boardEntity.user.id}">
		<a href="/board/${boardEntity.id}/upadteForm" class="btn btn-warning">수정</a>
	...
	<script>
      	async function deleteById(id) {
      		// 비동기 함수 호출 -> 비동기를 잘처리 하는 방법????
      		let response = await fetch("http://localhost:8080/board/" + id, {
      			method: "delete"
      		});
      		
      		// json() 함수는 json처럼 생긴 문자열을 자바스크립트 오브젝트로 변환
				...
				<li id="reply-${comment.id }"
					...
					<div class="d-flex">
						<div class="font-italic">작성자 : ${comment.user.username}
							&nbsp;</div>
						<button class="badge" id="reply"
							onClick="deleteById(${comment.id})">삭제</button>
					</div>
				</li>
			</c:forEach>
